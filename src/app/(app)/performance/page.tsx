...
export default function PerformancePage() {
    ...
    try {
      ...
      setReviews(
        Array.isArray(reviewsData)
          ? reviewsData
          : reviewsData.performances || [],
      );
      setInterns(
        Array.isArray(internsData) ? internsData : internsData.users || [],
      );
      if (userData.user) {
  ...
  const handleCreateReview = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDialogError(data.error || "Failed to create review");
      }
    } catch (err) {
      console.error("Failed to create review", err);
      setDialogError("Failed to create review. Please try again.");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleUpdateReview = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDialogError(data.error || "Failed to update review");
      }
    } catch (err) {
      console.error("Failed to update review", err);
      setDialogError("Failed to update review. Please try again.");
    } finally {
      setUpdating(false);
    }
  };
  ...
  const { avgPerformance, topPerformer, reviewsThisMonth } = useMemo(() => {
    const avg =
      interns.length > 0
        ? Math.round(
            interns.reduce((sum, i) => sum + (i.performanceScore || 0), 0) /
              interns.length,
          )
        : 0;
    const top = interns.reduce(
      (top, i) =>
        (i.performanceScore || 0) > (top?.performanceScore || 0) ? i : top,
      interns[0],
    );
        ...
        <Dialog open={isDialogOpen} onOpenChange={handleCloseDialog}>
          <DialogContent className="sm:max-w-[550px]">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingReview
                  ? "Modify the existing evaluation."
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setFormData({ ...formData, internId: v })
                    }
                  >
                    <SelectTrigger>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {formData.metrics.taskCompletion}
                    </span>
                  </div>
                  <Slider
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {formData.metrics.communication}
                    </span>
                  </div>
                  <Slider
                ...
                <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-lg text-center">
                  ...
                  <span
                    className={`text-lg font-bold ${getScoreColor(calculateAvgScore())}`}
                  >
                    {calculateAvgScore()}%
                  </span>
                </div>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                disabled={creating || updating || !formData.internId}
                className="text-black hover:text-black"
              >
                {(creating || updating) && (
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div
                className={`text-4xl font-bold ${getScoreColor(selectedIntern.performanceScore || 0)}`}
              >
              ...
              <Card key={review._id}>
                    ...
                    {(currentUser?.role === "founder" ||
                      ...
                      <div className="flex items-center gap-2">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 text-black hover:text-indigo-600"
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={review.metrics.taskCompletion * 10}
                      className="h-2"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={review.metrics.quality * 10}
                      className="h-2"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={review.metrics.communication * 10}
                      className="h-2"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={review.metrics.punctuality * 10}
                      className="h-2"
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={review.metrics.initiative * 10}
                      className="h-2"
                    />
                  </div>
                  {review.feedback && (
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">
                Track and evaluate intern performance.
              </p>
            </div>
            {(currentUser?.role === "founder" ||
              currentUser?.role === "teamlead") && (
            ...
            <TabsContent value="interns">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Click on an intern to view their detailed performance
                    history.
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {loading ? (
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {interns.map((intern) => {
                          const internReviewCount = reviews.filter(
                            (r) => r.intern._id === intern._id,
                          ).length;
                          return (
                            <TableRow
                              key={intern._id}
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Intern rankings based on weighted performance and task
                      completion.
                    </CardDescription>
                  </div>
                  <div className="flex items-center gap-2">
                        ...
                        {rankings.map((rank, index) => (
                          <TableRow key={rank.id}>
                            ...
                            <TableCell className="text-center">
                              <Badge
                                variant="outline"
                                className={getScoreBgColor(rank.avgPerformance)}
                              >
