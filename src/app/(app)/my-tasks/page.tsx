...
export default function MyTasksPage() {
  ...
  const [newTaskStatus, setNewTaskStatus] = useState<string>(""); // Main task status
  const [newStatus, setNewStatus] = useState<string>(""); // Intern status
  ...
  const handleOpenUpdate = (task: Task) => {
    ...
    setNewTaskStatus(task.status); // Set main task status
  ...
  const handleUpdateStatus = async () => {
    ...
    setSubmitting(true);
    try {
      // Update main task status (only interns can do this for their own tasks)
      const statusRes = await fetch("/api/tasks/" + selectedTask._id, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: newTaskStatus }),
      });

      // Update intern status and note
      const internRes = await fetch("/api/tasks/" + selectedTask._id + "/status", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!statusRes.ok || !internRes.ok) {
        // Rollback if failed
        setTasks(previousTasks);
        const statusError = await statusRes.json().catch(() => ({}));
        const internError = await internRes.json().catch(() => ({}));
        alert(statusError.error || internError.error || "Failed to update status");
      } else {
  ...
  const { todoTasks, inProgressTasks, reviewTasks, completedTasks, onHoldTasks } =
    useMemo(() => {
      return {
        ...
        onHoldTasks: tasks.filter((t) => t.status === "on_hold"),
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-5">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{onHoldTasks.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Click "Update Status" to report your progress
          </CardDescription>
        </CardHeader>
        <CardContent>
          {tasks.length === 0 ? (
              ...
              <TableBody>
                {tasks.map((task) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {getInternStatusBadge(task.internStatus)}
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
