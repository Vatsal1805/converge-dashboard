...
const navItems: NavItem[] = [
    ...
    { label: 'Dashboard', to: '/dashboard', icon: LayoutDashboard, roles: ['founder', 'teamlead', 'intern'] },
    ...
    { label: 'Team', to: '/team', icon: Users, roles: ['founder', 'teamlead', 'intern'] },
    { label: 'Users', to: '/dashboard/founder/users', icon: Users, roles: ['founder'] }, // Explicit link for users
...
export function AppSidebar({ user }: AppSidebarProps) {
    ...
    return (
        ...
        >
            {/* Logo */}
            <div className={cn('flex items-center gap-3 px-4 h-16 border-b border-slate-800 flex-shrink-0', collapsed && 'justify-center px-0')}>
                ...
                {!collapsed && (
                    <div className="overflow-hidden">
                        <p className="text-sm font-bold text-slate-100 leading-none truncate">ConvergeOS</p>
                        <p className="text-[10px] text-slate-400 mt-0.5 truncate">Converge Digitals</p>
                    </div>
                )}
            </div>
            ...
            <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1 custom-scrollbar">
                {visible.map(item => {
                    ...
                    const href = item.label === 'Dashboard' ? `/dashboard/${user.role}` : item.to;
                    const isActive = pathname === href || (item.to !== '/' && pathname.startsWith(item.to));
                    ...
                    return (
                        ...
                        >
                            <item.icon className={cn('h-4 w-4 flex-shrink-0 transition-colors', isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-200', collapsed ? 'mx-auto' : '')} />
            ...
            <div className="p-3 border-t border-slate-800 bg-slate-900/50">
                {!collapsed && (
                    <div className="mb-3 px-1">
                        ...
                        <div className="text-[10px] text-slate-500 capitalize truncate">{user.role} • {user.department}</div>
                    </div>
                )}
                <Button
                    variant="ghost"
